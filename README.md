...
- выбор порога уверенности распознавания
...
В наименовании фото присутствует номер табло, заданный префикс и постфикс с порядковым номером изображения объекта, если есть больше одного для данного объекта. Есть ~10 фотографий, на которых табло отсутствует.
...
- Рассчитываем confusion matrix и анализируем на каких символах ошибается чаще всего.
...
- переименование файлов
- печать датафрейма с результатами в файл xlsx.
- создание csv файл с разметкой - имя_изображения,распознанный текст

## Выбор порога уверенности переименования
Произведён запуск скрипта целиком на папке с изображениями тестовой выборки. Порог общей уверенности распознавания, при котором происходит отметка переименования как ненадёжного (!_ перед новым именем) выбран 0.8.
Созданный файл логов: [logs.xlsx](logs.xlsx).

<img src="images/logs.png" alt="Порог" width="800">

Время отработки на 220 изображениях составило 165 с, т.е. на 1000 изображений можно ожидать время выполнения около 12,5 минут.


С помощью [threshold_precision.py](src/threshold_preciion_ratio.py) к результатам отработки скрипта добавлены истинные номера на изображениях. Построен график зависимости precison от порога уверенности. 

<img src="images/threshold.png" alt="Порог" width="450">

Оставляем порог уверенности 0.8
Запуск проекта собран в файл .exe. 
 
# Итоговый вывод
Создан инструмент для автопереименования изображений в соответствии с номером на табло, находящемся на изображении.

Для модели детекции табло accuracy на тестовой выборке составляет 96%, для модели распознавания номера - 75%.
Выбран порог уверенности надёжности для переименования 0.8.
Даже при высоком значении порога уверенности не удаётся достичь 100% точности переименования.

Для улучшения качества детекции требуется доразметить больше изображений для обучающей выборки.
Улучшить качество распознавания может помочь большее количество примеров изображений для обучения.

Рекомендуется использовать созданный функционал автопереименования как вспомогательный для уменьшения времени ручного переименования с последующей валидацией.
